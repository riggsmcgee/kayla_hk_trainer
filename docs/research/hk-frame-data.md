...
| Jump hold (variable height) | vy is pinned at 16.65 for up to **9 steps (0.18 s)** while the button is held (`JUMP_STEPS = 9`); minimum **4 steps (0.08 s)** (`JUMP_STEPS_MIN = 4`). On release after the minimum, **vy is set to 0 instantly** (`JumpReleased()`), then gravity takes over — this is why short hops feel so crisp. **Clarification (Session 5, verified against the decompiled source):** `JumpReleased()` is called from `LookForInput()` on every Update frame in which the button is simply not held, and its only gates are `velocity.y > 0` and `jumped_steps >= JUMP_STEPS_MIN`; `CancelJump()` resets `jump_steps` but never `jumped_steps`, so the cutoff is honoured for the **whole ascent**, not just the pinned phase — jump height is continuous from the 4-step minimum to the full held jump. | decompiled `HeroController.cs` (`Jump()`, `JumpReleased()`, `CancelJump()`) | decompiled |
...
| Interruptible | Dashing (or a head bump) cancels the bounce early. You cannot jump or double-jump out of a bounce (`CanJump`/`CanDoubleJump` require `!cState.bouncing`), and a jump release never cuts it — `FixedUpdate` re-pins `BOUNCE_VELOCITY` every step while bouncing. *(Corrected Session 5.)* | decompiled code | decompiled |
